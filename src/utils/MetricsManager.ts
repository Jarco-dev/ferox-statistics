...
class MetricsManager {
    ...
    constructor(client: typeof Client) {
        ...
        this.port = client.sConfig.MISC.METRICS_PORT;
        this.metrics = {};
        ...
        this.app.get("/metrics", async (req, res): Promise<void> => {
            try {
                ...
                this.logger.error("Error while handeling metrics request", err);
                res.status(500).end();
            }
        });
    }
    ...
    private _createMetrics(): void {
        ...
        this.metrics.clientCacheSize = new this.promClient.Gauge({
            ...
            collect: () => {
                ...
                this.metrics.clientCacheSize.labels("members")
                    .set(this.client.guilds.cache.reduce((a, g) => a += g.members.cache.size, 0));
                this.metrics.clientCacheSize.labels("messages")
                    .set(this.client.channels.cache.reduce((a, c) => a += (c.isText()) ? c.messages.cache.size : 0, 0));
            }
        });
    }
    ...
    private _createListeners(): void {
        ...
        this.client.on("interactionCreate", (i) => {
            if (i.isCommand() || i.isContextMenu()) this.metrics.commandsExecuted.labels(i.commandName).inc();
            else if (i.isButton()) this.metrics.buttonsExecuted.labels(i.customId).inc();
            else if (i.isSelectMenu()) this.metrics.menusExecuted.labels(i.customId).inc();
        });
    }
}
